...
}.toList().map { it.get()  /*gotta get, or else exceptions will not be handled*/ }
...
fun <K, V> Sequence<K>.parAssociateWith(op: (K) -> V): Map<K, V> {
    ...
    return futures.associate { it.first to it.second.get()  /*gotta get, or else exceptions will not be handled*/ }
